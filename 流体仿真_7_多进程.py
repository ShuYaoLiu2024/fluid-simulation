...
def process_manage_ini(fluidcell_all):
	...
	for i in range(fluidcell_all_len):
		for j in range(i+1,fluidcell_all_len):
			tasks.append([i,j])
	a=math.floor(len(tasks)/process_number)+1#向下取整
...
def process_manage(fluidcell_all,process_plan):
	#time0=time.time()
	fluidcell_all_process=[]
	fluidcell_all_process_done=[]
	...
	for i in range(process_number):
		fluidcell_all_process.append(copy.deepcopy(fluidcell_all))
	...
	for i in range(process_number):
		...
		fluidcell_all_process_done_one = pool.apply_async(renew_fpfv_one, (fluidcell_all_process[i],process_plan[i])).get()#耗时长
		print(time.time()-time0)
		fluidcell_all_process_done.append(fluidcell_all_process_done_one)
	...
	for i in range(len(fluidcell_all)):
		for j in range(process_number):
			fluidcell_all[i].Fp=fluidcell_all[i].Fp+fluidcell_all_process_done[j][i].Fp
	print(time.time()-time0)
	...
	for i in range(process_number):
		process_one=Process(target=renew_fpfv_one, args=(fluidcell_all_process[i],process_plan[i]))
	...
	for i in range(len(fluidcell_all)):
		for j in range(process_number):
			fluidcell_all[i].Fp=fluidcell_all[i].Fp+fluidcell_all_process[j][i].Fp

	'''

	#renew_fpfv_one(fluidcell_all,process_plan[0])
...
def draw(area):
	...
	def draw_update(frame):
		...
		fluidcell_all,gen=main_update(fluidcell_all,gen,process_plan)
...
def main_update(fluidcell_all,gen,process_plan):
	gen+=1

	process_manage(fluidcell_all,process_plan)
