...
mu=0.8#粘度
rho=1#即每个微团的质量
k_attraction=1000#分子间排斥力常数
k_attraction_r=80#分子间吸引力常数
fluidcell_number=80

area=[20,20]#区域长宽
k_area=0.4

process_worker_number=9
	...
	if fluidcell_b.contral==0:
		if r1<4:
			...
			if r<1:
				...
				fp=k_attraction_r*(r*r*r-301/80*r*r+343/80*r-61/40)
		...
		Fp_a=np.multiply(R,[fp/r])
		#Fp_a=np.multiply([R[0],abs(R[1])],[0,fp/r])
		Fp_b=np.array([0,0])
...
def renew_fpfv_one(fluidcell_all_process_one,tasks_one):
	...
	for task in tasks_one:
		...
		p=math.sqrt(sum(np.multiply(Fp_a,Fp_a)))

		fluidcell_all_process_one[task[0]].p=fluidcell_all_process_one[task[0]].p+p  
		fluidcell_all_process_one[task[1]].p=fluidcell_all_process_one[task[1]].p+p  
	...
	while True:
		...
		for process_worker_id in range(process_worker_number):
			...
			for i in range(len(rec)):
				...
				fluidcell_all[i].p=fluidcell_all[i].p+rec[i].p
			time_record.append(time.time()-time1)
		
		#处理数据：计算R，区域内，压力显示
		p_min=100000
		p_max=0
		for i in range(len(fluidcell_all)-1):
		...
		d_p=p_max-p_min+0.01
		for fluidcell in fluidcell_all:
			in_area(fluidcell)
			fluidcell.p=(fluidcell.p-p_min)/d_p
...
def draw(fluidcell_all):
	...
	points_anis=[]
	for i in range(len(fluidcell_all)):
		points_anis.append(plt.plot([],[],'o')[0])
	#points_ani = plt.scatter([],[],c=[])
	ctrl_ani = plt.plot([],[],'o',20,color='gray',alpha=0.3,markersize=25)[0]
	...
	def draw_update(frame):
		...
		for i in range(len(fluidcell_all[:-1])):
			fluidcell=fluidcell_all[i]
			points_anis[i].set_data(fluidcell.R[0],fluidcell.R[1])
			points_anis[i].set_color(plt.get_cmap('cool')(fluidcell.p))
